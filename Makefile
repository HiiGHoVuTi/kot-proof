...
install-deps:
	...
	@if opam list --switch=$(SWITCH_NAME) --installed coq | grep -q "8.20.1" && \
       opam list --switch=$(SWITCH_NAME) --installed coq-iris | grep -q "dev.2025-07-23.1.5cd7c971" && \
       ...
       opam list --switch=$(SWITCH_NAME) --installed coq-stdpp | grep -q "dev.2025-07-21.0.7c925a6f"; \
	then \
		...
		opam exec --switch=$(SWITCH_NAME) -- opam install -y coq.8.20.1 coq-iris.dev.2025-07-23.1.5cd7c971 coq-iris-heap-lang coq-aac-tactics coq-stdpp.dev.2025-07-21.0.7c925a6f; \
	fi
